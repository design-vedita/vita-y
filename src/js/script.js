    ...
    function addedItem(el, el_add, parent) {
        ...
        for(i = 0; i < item.length; i++) {
            ...
            item[i].onclick = function(e){
                ...
                // Чтобы дважды не добавлять один и тот же товар в ячейку, делаем проверку на
                // наличие класса
                if (!self.classList.contains('no--add')) {
                    ...
                    if(self.classList.contains('js-elem-work')) {
                ...
                // Добавляем класс, чтобы один и тот же элемент несколько раз не добавлять
                setTimeout(function(){
                        self.classList.add('no--add');
                }, 200)
    ...
    function deleteItem(el) {
        ...
        for (i = 0; i < delItem.length; i++) {
            ...
            delItem[i].onclick = function(e){
                ...
                var searchImage = image.getAttribute('src'),
                    splitUrl = searchImage.slice(0, -10),
                    searchUrl = splitUrl + '.svg';
                ...
                if (parent.classList.contains('js-added-work')) {
                    ...
                    var img = blockWork.querySelectorAll('img');

                    for (k = 0; k < img.length; k++) {
                        var src = img[k].getAttribute('src');

                        if (src == searchUrl) {
                            img[k].parentNode.classList.remove('no--add');
                        }
                    }
